...
export default function EprModal({
  ...
  const handleSubmit = async () => {
    if (epr) {
      console.log("Updating EPR", epr.id);
      ...
      <div className="bg-white rounded-2xl p-7 w-[480px] shadow-xl">
        ...
        <div className="mb-6">
          ...
          <select
            value={status}
            onChange={(e) =>
              setStatus(e.target.value as "draft" | "submitted" | "archived")
            }
            className="border rounded-lg p-2 w-full mt-1"
          >
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={async () => {
              const res = await api.post("/api/epr/assist", {
                overallRating,
                technicalSkillsRating: technicalRating,
                nonTechnicalSkillsRating: nonTechnicalRating,
              });

              setRemarks(res.data.suggestedRemarks);
            }}
            className="px-3 py-2 text-sm bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg transition"
          > 
          AI Remarks
          </button>
